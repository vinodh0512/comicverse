...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; }
        ...
        .user-text { color: #fff; font-size: 14px; font-weight: 700; cursor: pointer; }
        ...
        .enhanced-card:hover { transform: translateY(-8px); box-shadow: 0 10px 30px rgba(0,0,0,0.5); border-color: #333; }
        
        .enhanced-card-image { 
            width: 100%; aspect-ratio: 2/3; background-color: #252525; 
        ...
        .enhanced-card-title { font-size: 15px; font-weight: 700; text-transform: uppercase; margin-bottom: 5px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.4; color: #fff; }
        ...
        .rating { color: #ffcc00; font-weight: bold; display: flex; align-items: center; gap: 4px; }
    ...
    <nav class="navbar">
        ...
        <?php if(isset($_SESSION['username'])): ?>
            <div class="user-menu user-dropdown">
                <div class="user-text">Hi, <span><?php echo htmlspecialchars($_SESSION['username']); ?></span> <i class="fas fa-caret-down"></i></div>
    ...
    <script>
        ...
        async function fetchWebtoons() {
            ...
            try {
                ...
                if (res.ok) { 
                    ...
                    if (json && json.status==='success' && Array.isArray(json.stories)) stories = json.stories; 
                }
                
                // Fallback API if Primary is empty
                if (!stories.length) {
                ...
                grid.innerHTML = '<div class="no-content" style="color:red">Unable to load content.</div>';
            }
        }
        ...
        function loadMore() {
            ...
            nextBatch.forEach(story => {
                // Ensure link includes series and type properly
        ...
        function initLazyImages(root){
            ...
            if (!('IntersectionObserver' in window)) { 
                images.forEach(img => { img.src = img.getAttribute('data-src'); img.classList.remove('lazy-img'); }); 
                return; 
            }
            if (!io) io = new IntersectionObserver((entries) => {
                entries.forEach(entry => { 
                    if (entry.isIntersecting) { 
                        ...
                        if (src) { img.src = src; img.removeAttribute('data-src'); img.classList.remove('lazy-img'); io.unobserve(img); } 
                    } 
                });
