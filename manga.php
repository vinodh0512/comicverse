...
<head>
    ...
    <meta name="description" content="Read popular manga on ComicVerse. Explore Shonen, Seinen, and more.">
    <link rel="canonical" href="http://localhost/comicverse/manga.php">
    <link rel="preconnect" href="https://cdnjs.cloudflare.com" crossorigin>
    <title>Read Manga Online | ComicVerse</title>
...
<body>
    ...
    <div class="page-header">
        <div class="header-title">
            <h1>Popular <span>Manga</span></h1>
            <p>Read the latest hits from Japan. Shonen, Seinen, and more.</p>
        </div>
        <div class="search-wrapper">
            <input type="text" id="searchInput" class="search-input" placeholder="Search manga..." onkeyup="filterManga()">
            <i class="fas fa-search search-icon"></i>
        </div>
    </div>

    <div class="container">
        <div class="update-grid" id="mangaGrid">
    ...
    <script>
        // --- STATE ---
        let allManga = [];         // Stores ALL data of type 'manga'
        let filteredManga = [];    // Stores current search results
        ...
        async function fetchManga() {
            ...
            try {
                ...
                if (!stories.length) { grid.innerHTML = '<div class="no-content" style="color:red">No manga found.</div>'; return; }
                ...
                allManga = norm.filter(s => s.type === 'manga');
                filteredManga = allManga;
                grid.innerHTML = '';
                if (allManga.length === 0) grid.innerHTML = '<div class="no-content">No manga uploaded yet.</div>'; else loadMore();
            } catch (error) {
                console.error(error);
                grid.innerHTML = '<div class="no-content" style="color:red">Error loading manga.</div>';
            }
        }
        ...
        function filterManga() {
            ...
            if (query === "") {
                ...
                filteredManga = allManga.filter(story => 
            ...
            if (filteredManga.length === 0) {
                document.getElementById('mangaGrid').innerHTML = '<div class="no-content">No manga matched your search.</div>';
            } else {
                loadMore();
            }
        }
        ...
        function loadMore() {
            ...
            const nextBatch = filteredManga.slice(currentIndex, currentIndex + itemsPerPage);
            ...
            if (currentIndex >= filteredManga.length) {
