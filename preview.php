...
// --- 2. LIKES STORAGE (Fixed Permissions Logic) ---
    ...
    try {
        if ($action === 'toggle') {
            ...
            if ($liked) { 
                ...
                $data['users'][$userName] = date('Y-m-d H:i:s'); 
                $liked = true; 
            }
            
            if(save_likes($likesFile, $data) === false) {
                throw new Exception("Write failed");
            }
        }
    } catch(Exception $e){
        // In production, log this error to a file
    }
        ...
        if (file_exists($jsonPath)) {
            ...
            if ($data) {
                ...
                if ($uploadTime >= $latestTime) {
                    ...
                    $meta['description'] = $data['meta']['description'] ?? $meta['description']; // Get actual description
...
<head>
    ...
    <style>
        ...
        .btn-login:hover { background: #ff333b; transform: translateY(-2px); }
        ...
        .user-avatar { width: 36px; height: 36px; background: linear-gradient(135deg, var(--primary), #800000); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; color: white; font-size: 14px; }
        .user-name { font-weight: 600; font-size: 14px; color: var(--text-main); display: block; }
        .fa-chevron-down { font-size: 12px; color: var(--text-muted); }
        ...
        .dropdown-menu.show { display: flex; animation: slideDown 0.2s ease-out; }
        ...
        .cover-box { flex: 0 0 350px; position: relative; }
        ...
        .series-title { font-size: 48px; font-weight: 800; line-height: 1.1; margin-bottom: 20px; background: linear-gradient(to right, #fff, #aaa); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        ...
        .desc-text { color: var(--text-muted); line-height: 1.6; font-size: 16px; margin-bottom: 30px; max-width: 700px; }
        ...
        .btn-main:hover { background: #ff333b; transform: translateY(-2px); box-shadow: 0 6px 20px rgba(236,29,36,0.6); }
        ...
        .btn-sec:hover { border-color: var(--primary); color: var(--primary); background: rgba(236,29,36,0.05); }
        ...
        .review-card { background: var(--bg-card); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color); }
        ...
        @media (max-width: 900px) {
            ...
            .nav-center, .user-name { display: none; }
...
<body>
    ...
    <div class="preview-container">
        <div class="cover-box">
            <img src="<?php echo $meta['cover']; ?>" alt="cover">
        </div>
        <div class="info-box">
            <div class="meta-tags">
            ...
            <div class="actions">
                <a href="<?php echo $readLink; ?>" target="<?php echo $readLinkTarget; ?>" class="btn-main">
                    <i class="fas fa-<?php echo $btnIcon; ?>"></i> <?php echo $btnText; ?>
                </a>
                
                <button class="btn-sec" id="btnList">
        ...
        <div class="review-form-box">
            ...
            <?php if($userName): ?>
                <div style="margin-bottom:15px;" id="starContainer">
                    <i class="fas fa-star star-input" data-val="1"></i>
                    <i class="fas fa-star star-input" data-val="2"></i>
                    <i class="fas fa-star star-input" data-val="3"></i>
                    <i class="fas fa-star star-input" data-val="4"></i>
            ...
            <?php else: ?>
                <p style="color:#aaa;">Please <a href="login.php" style="color:var(--primary)">login</a> to write a review.</p>
            <?php endif; ?>
        </div>
        
        <div>
            <h3 style="margin-bottom:20px;">Community Reviews <span id="avgRating" style="font-size:14px; color:#ffcc00; margin-left:10px;"></span></h3>
            <div class="review-list" id="reviewList">
                <div style="text-align:center; padding:20px; color:#555;">Loading reviews...</div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // --- 2. LIST LOGIC ---
        // Checks if list.php exists and handles adding/removing
        async function initList() {
            ...
            try {
                ...
                btn.onclick = async () => {
                    ...
                    const fd = new FormData();
                    fd.append('action', isAdding ? 'add' : 'remove');
                    fd.append('series', CFG.series);
                ...
                console.log('List system unavailable'); 
                btn.style.display = 'none'; // Hide if list.php is missing
            }
        }
            ...
            try {
                ...
                if (data.status === 'success') {
                    ...
                    if(data.summary.total > 0) {
                        document.getElementById('avgRating').innerHTML = `<i class="fas fa-star"></i> ${data.summary.average} (${data.summary.total})`;
                    }

                    // Render List
                    const list = document.getElementById('reviewList');
                    if (data.reviews.length === 0) {
                        list.innerHTML = '<div style="text-align:center; padding:20px; color:#777;">Be the first to review!</div>';
                        return;
                    }
                    ...
                    list.innerHTML = data.reviews.map(r => `
                        <div class="review-card">
                            <div class="r-header">
                                <span class="r-user">${escapeHtml(r.username)}</span>
                                <span class="r-date">${r.timestamp.split(' ')[0]}</span>
                            </div>
            ...
            subBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if(j.status === 'success') {
                        ...
                        subBtn.textContent = 'Posted!';
                    } else {
                        alert(j.message);
                        subBtn.textContent = 'Post Review';
