...
<head>
    ...
    <meta name="description" content="Read digital comics and graphic novels on ComicVerse. Explore western-style comics, one-shots, and more.">
    <link rel="canonical" href="http://localhost/comicverse/comic.php">
    <link rel="preconnect" href="https://cdnjs.cloudflare.com" crossorigin>
    <title>Read Comics Online | ComicVerse</title>
...
<body>
    ...
    <nav class="navbar">
        ...
        <div class="nav-links" role="navigation" aria-label="Primary">
            ...
            <a href="comic.php" class="active">Comics</a>
    ...
    <div class="page-header">
        <div class="header-title">
            <h1>Digital <span>Comics</span></h1>
            <p>Western style comics, graphic novels, and one-shots.</p>
        </div>
        <div class="search-wrapper">
            <input type="text" id="searchInput" class="search-input" placeholder="Search comics..." onkeyup="filterComics()">
            <i class="fas fa-search search-icon"></i>
        </div>
    </div>

    <div class="container">
        <div class="update-grid" id="comicGrid">
    ...
    <script>
        // --- STATE ---
        let allComics = [];     // Stores ALL data of type 'comic'
        let filteredComics = [];   // Stores current search results
        ...
        async function fetchComics() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (json && json.status === 'success' && Array.isArray(json.stories)) {
                        stories = json.stories;
                    }
                }
                // Fallback to index ajax if needed
                if (!stories.length) {
                    const res2 = await fetch('index.php?ajax=1&t=' + Date.now());
                    if (res2.ok) {
                        const j2 = await res2.json();
                        if (j2 && j2.status === 'success' && Array.isArray(j2.data)) {
                            stories = j2.data.map(s => ({
                                title: s.title,
                                folder: s.folder,
                                type: s.type,
                                latest_chapter: s.chapter_num,
                                time_ago: s.time_ago,
                ...
                if (!stories.length) { grid.innerHTML = '<div class="no-content" style="color:red">No comics found.</div>'; return; }
                // Normalize and filter
                const norm = stories.map(story => ({
                    title: story.title || 'Untitled',
                    folder: story.folder || '',
                    type: (story.type || '').toLowerCase(),
                    latest_chapter: story.latest_chapter || story.chapter_num || 1,
                    time_ago: story.time_ago || 'just now',
                    thumbnail: story.thumbnail || 'https://via.placeholder.com/400x600?text=No+Cover',
                    rating: story.rating || '0.0 (0%)'
                }));
                allComics = norm.filter(s => s.type === 'comic');
                filteredComics = allComics;
                grid.innerHTML = '';
                if (allComics.length === 0) { grid.innerHTML = '<div class="no-content">No comics uploaded yet.</div>'; }
                else { loadMore(); }
            } catch (error) {
                console.error(error);
                grid.innerHTML = '<div class="no-content" style="color:red">Error loading comics.</div>';
            }
        }
        ...
        function filterComics() {
            ...
            if (query === "") {
                ...
                filteredComics = allComics.filter(story => 
            ...
            if (filteredComics.length === 0) {
                document.getElementById('comicGrid').innerHTML = '<div class="no-content">No comics matched your search.</div>';
            } else {
                loadMore();
            }
        }
        ...
        function loadMore() {
            ...
            const nextBatch = filteredComics.slice(currentIndex, currentIndex + itemsPerPage);
            
            nextBatch.forEach(story => {
                // NOTE: Assumes story.folder, story.type, story.thumbnail, story.latest_chapter, story.time_ago, story.rating exist in API response
            ...
            if (currentIndex >= filteredComics.length) {
